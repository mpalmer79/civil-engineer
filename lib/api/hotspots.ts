...
export async function getHotspots(): Promise<DemoSourced<Hotspot[]>> {
  const result = await apiFetch<ApiHotspot[]>(
  ...
  if (!result.ok) return { data: staticHotspots, source: "demo_fixture" };
  const data = result.data;
  return { source: "backend_seeded", data: data.map((h) => ({
    id: h.hotspot_id,
    name: h.name,
    category: h.category,
