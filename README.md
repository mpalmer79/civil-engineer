...
This is a real-world foundation, not a full production release. The production foundation work has already added local authentication and role-based access control, durable object storage support, a production database foundation (Postgres support and an Alembic migration framework, while keeping SQLite for local development and tests), and an account-lifecycle and billing foundation: password reset and organization team invitations with real email delivery through an SMTP provider (a no-op mailer is the default for local development), Stripe checkout for the professional plan with signature-verified, idempotent webhooks, and usage metering that is advisory by default and enforceable for selected categories. Email and billing are inactive until configured, and the public Brookside demo is never billed or enforced. See `docs/AUTH_LIFECYCLE.md`, `docs/EMAIL_DELIVERY.md`, `docs/STRIPE_BILLING.md`, and `docs/BILLING_AND_USAGE.md`. Work that remains out of scope here includes enterprise single sign-on, email confirmation, production security hardening, a full applicant portal, OCR, live AI calls, and real plan rendering. CAD scope beyond DXF metadata (DWG, Autodesk, Civil 3D, GIS, and computer vision) is also future work.
