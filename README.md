...
This is a real-world foundation, not a full production release. The production foundation work has already added local authentication and role-based access control, durable object storage support, a production database foundation (Postgres support and an Alembic migration framework, while keeping SQLite for local development and tests), and an account-lifecycle and billing-readiness foundation: password reset, organization team invitations, code-defined subscription plans, and advisory usage metering. Billing is honestly inactive (Stripe checkout and webhooks are deferred and no payment is collected), and no real email is sent yet (a no-op mailer stands in). See `docs/AUTH_LIFECYCLE.md` and `docs/BILLING_AND_USAGE.md`. Work that remains out of scope here includes enterprise single sign-on, email confirmation, live Stripe billing, production security hardening, a full applicant portal, OCR, live AI calls, and real plan rendering. CAD scope beyond DXF metadata (DWG, Autodesk, Civil 3D, GIS, and computer vision) is also future work.
