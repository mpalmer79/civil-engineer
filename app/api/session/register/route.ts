...
// backend and establishes the HttpOnly cookie session in the same step, with
// the cookie lifetime derived from the backend's expires_in_minutes. The
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!upstream.ok || !payload.access_token) {
    return NextResponse.json(
      { detail: payload.detail ?? "Registration failed.", request_id: requestId },
