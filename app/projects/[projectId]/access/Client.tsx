...
export default function ProjectAccessPageClient({ projectId }: { projectId: string }) {
  ...
  const handleGrant = async () => {
    ...
    setError(null);
    setMessage(null);
    const result = await grantProjectAccess(projectId, {
      accessLevel,
      userId: userId.trim(),
    });
    setBusy(false);
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        <Link href={`/projects/${projectId}`} className="nav-link">
          Back to project
        </Link>
