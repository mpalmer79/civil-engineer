...
export default async function EvaluationPage() {
  ...
  const planSummaryResult = await getPlanConsistencySummary();
  if (!planSummaryResult.ok) {
    return (
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <RequestFailureCard failure={planSummaryResult} />
      </div>
    );
  }
  const planSummary = planSummaryResult.data;
  return (
    <div>
      <PageHeader
        eyebrow="Evaluation dashboard"
        title="Evaluation-driven AI design"
        description="Civil Engineer AI scores AI draft findings against the expected Brookside Meadows findings. The dashboard reports recall, precision, citation validity, human review required rate, and validation and safety failure counts."
      />

      <div className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6 lg:px-8">
        <SectionCard title="Evaluation scoring">
        ...
        {planSummary ? (
          <SectionCard title="Plan sheet and CAD-aware review">
            <p className="text-sm text-slate-600">
              Plan consistency metrics from the seeded plan references
        ...
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
          ...
          plausible reference values. The scoring panel above computes
