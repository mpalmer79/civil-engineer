...
| Homepage case-study facts | Seeded demo | `app/page.tsx` counts the fixture arrays in `data/*.ts` | The homepage shows fixed case-study facts counted from the seeded fixture and labeled as such; it renders no operational widgets | Live operational metrics stay behind sign-in on `/dashboard` |
